<script lang="ts">
    ...
    let daysInMonth = $derived(new Date(currentYear, selectedMonth + 1, 0).getDate());
...
<style>
    .date-widget { display: flex; gap: 0.5rem; align-items: center; }
    select { padding: 0.4rem; border-radius: 4px; border: 1px solid #ccc; }
</style>